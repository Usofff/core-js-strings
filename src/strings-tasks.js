...
function encodeToRot13(str) {
  const encode = (char) => {
    ...
    const alphabetLower =
      'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
    if (!(alphabetUpper.includes(char) || alphabetLower.includes(char))) {
      return char;
    }
    if (char.toUpperCase() === char) {
      return alphabetUpper[alphabetUpper.indexOf(char) + 13];
    }
    return alphabetLower[alphabetLower.indexOf(char) + 13];
  };
  return str
    .split('')
